...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    private static Logger logger = LoggerFactory.getLogger(UpmsSessionDao.class);
    /**
     * 会话key
     */
    private final static String LAMBO_UPMS_SHIRO_SESSION_ID = "lambo-upms-shiro-session-id";
    /**
     * 全局会话key
     */
     ...
     * 局部会话key
     */
    private final static String LAMBO_UPMS_CLIENT_SESSION_ID = "lambo-upms-client-session-id";
    /**
     * 单点同一个code所有局部会话key
     */
    ...
    protected Serializable doCreate(Session session) {
        ...
        RedisUtil.set(LAMBO_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected Session doReadSession(Serializable sessionId) {
        String session = RedisUtil.get(LAMBO_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            String code = RedisUtil.get(LAMBO_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            Jedis jedis = RedisUtil.getJedis();
            jedis.del(LAMBO_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            jedis.srem(LAMBO_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId);
            jedis.close();
        }
        if ("server".equals(upmsType)) {
            // 当前全局会话code
            String code = RedisUtil.get(LAMBO_UPMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除全局会话
            RedisUtil.remove(LAMBO_UPMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除code校验值
            RedisUtil.remove(LAMBO_UPMS_SERVER_CODE + "_" + code);
            // 清除所有局部会话
            Jedis jedis = RedisUtil.getJedis();
            Set<String> clientSessionIds = jedis.smembers(LAMBO_UPMS_CLIENT_SESSION_IDS + "_" + code);
            for (String clientSessionId : clientSessionIds) {
                jedis.del(LAMBO_UPMS_CLIENT_SESSION_ID + "_" + clientSessionId);
                jedis.srem(LAMBO_UPMS_CLIENT_SESSION_IDS + "_" + code, clientSessionId);
            }
            logger.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(LAMBO_UPMS_CLIENT_SESSION_IDS + "_" + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
            RedisUtil.lrem(LAMBO_UPMS_SERVER_SESSION_IDS, 1, sessionId);
        }
        // 删除session
        RedisUtil.remove(LAMBO_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            RedisUtil.set(LAMBO_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
